...
class WaterConsumptionSqliteRepositoryImpl implements WaterConsumptionRepository {
  final Database _db;
  ...
  Future<List<WaterConsumptionHistory>> getHistory() async {
    final result = await _db.query(
      _tableName,
      orderBy: '$_dateKey DESC',
    );
    
    return result.map((record) => WaterConsumptionHistory(
  ...
  Future<WaterConsumption> getCurrentConsumption() async {
    ...
    if (result.isEmpty) {
      return WaterConsumption(consumed: 0, target: _defaultTarget);
    }
